...
--    Les caractères accentués sont écrits via chr(<code Unicode>) plutôt
--    qu'en clair : un fichier contenant des accents littéraux peut être
--    corrompu en traversant certains terminaux/pipes (vécu avec PowerShell
--    sur Windows) — chr() les rend immunisés à ce type de corruption, le
--    fichier lui-même ne contient plus que de l'ASCII.
    ...
    regexp_replace(
      lower(
        translate(
          ...
          chr(221)||chr(199)||chr(209),
          'aaaaaa' || 'eeee' || 'iiii' || 'ooooo' || 'uuuu' || 'ycn' ||
          'AAAAAA' || 'EEEE' || 'IIII' || 'OOOOO' || 'UUUU' || 'YCN'
        )
      ),
      '[^a-z0-9]+', '-', 'g'
