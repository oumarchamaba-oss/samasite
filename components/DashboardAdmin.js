...
export default function DashboardAdmin() {
  ...
  const [paiementBoutiqueOuvert, setPaiementBoutiqueOuvert] = useState(null);
  const [paiementBoutiqueCle, setPaiementBoutiqueCle] = useState("");
  const [paiementBoutiqueTraitement, setPaiementBoutiqueTraitement] = useState(false);
  const [paiementBoutiqueErreur, setPaiementBoutiqueErreur] = useState("");
  ...
  const activerPaiementBoutique = async (client) => {
    ...
    setPaiementBoutiqueTraitement(false);
    if (error) { setPaiementBoutiqueErreur("Impossible d'activer le paiement en ligne : " + (error.message || "erreur inconnue")); return; }
    setPaiementBoutiqueOuvert(null);
    setPaiementBoutiqueCle("");
    chargerClients();
  };
  const desactiverPaiementBoutique = async (client) => {
    setPaiementBoutiqueTraitement(true);
    const { error } = await supabase.rpc("retirer_paiement_en_ligne_admin", { p_site_id: client.id });
    setPaiementBoutiqueTraitement(false);
    if (!error) chargerClients();
  };
                  ...
                  {c.paiement_en_ligne_demande && (
                    c.paiement_en_ligne_actif ? (
                      ...
                      <div className="flex flex-col items-end gap-1">
                        ...
                        {paiementBoutiqueErreur && <span className="text-xs" style={{ color: "#F87171" }}>{paiementBoutiqueErreur}</span>}
                      </div>
                    ) : (
                      <button onClick={() => { setPaiementBoutiqueOuvert(c.id); setPaiementBoutiqueCle(""); setPaiementBoutiqueErreur(""); }} aria-label="Activer le paiement en ligne demandé" title="Activer le paiement en ligne demandé"
                        className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-bold" style={{ background: "rgba(250,204,21,0.18)", color: T.jaune }}>
                        <Wallet size={11} /> Paiement en ligne demandé — activer
                      </button>
                    )
                  )}
                  <div className="flex items-center gap-1.5">
