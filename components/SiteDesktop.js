    ...
    <div className="w-full overflow-hidden" style={{ background: T.blanc }}>
      ...
      <header className="sticky top-0 z-20 px-5 md:px-10 py-3.5" style={{ background: "rgba(255,255,255,0.96)", borderBottom: `1px solid ${T.bleuClairBord}`, backdropFilter: "blur(10px)" }}>
        <div className="max-w-6xl mx-auto flex items-center justify-between gap-4">
          ...
          <nav className="hidden md:flex items-center gap-7">
            <a href="#accueil" className="text-sm font-medium" style={{ color: T.encre }}>Accueil</a>
            <a href="#produits" className="text-sm font-medium" style={{ color: T.gris }}>{catalogueLabel}</a>
            <a href="#contact" className="text-sm font-medium" style={{ color: T.gris }}>Contact</a>
          </nav>
          <a href={waGeneral} target="_blank" rel="noopener noreferrer" className="shrink-0 flex items-center gap-2 rounded-full px-4 py-2.5 text-xs md:text-sm font-bold text-white" style={{ background: "#25D366" }}>
      ...
      <section id="accueil" className="scroll-mt-20">
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 min-h-[480px] items-stretch">
          <div className="px-6 md:px-10 py-14 md:py-20 flex flex-col justify-center">
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              <a href={waGeneral} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full px-6 py-3.5 text-sm font-bold text-white" style={{ background: heroFonce }}>
                <MessageCircle size={17} /> {actionLabel} sur WhatsApp
              </a>
            </div>
          </div>
          <div className="relative min-h-[300px] md:min-h-0 overflow-hidden md:rounded-l-[2rem]" style={{ background: p.fond }}>
            {business.banniere ? (
        ...
        <div className="max-w-6xl mx-auto px-5 md:px-10 py-14 md:py-16">
          ...
          {categories.length > 1 && (
            <div className="flex items-center gap-7 overflow-x-auto pb-3 mb-8 border-b" style={{ borderColor: T.bleuClairBord, scrollbarWidth: "none" }}>
              {categories.map((cat) => (
                <button key={cat} onClick={() => setCategorieActive(cat)} className="shrink-0 text-sm font-medium pb-3 -mb-3 border-b-2" style={{ color: categorieActive === cat ? p.primaire : T.gris, borderColor: categorieActive === cat ? p.primaire : "transparent" }}>{cat}</button>
              ))}
            </div>
          )}
          ...
          <div className="flex gap-5 overflow-x-auto pb-3 snap-x" style={{ scrollbarWidth: "none" }}>
            {itemsAffiches.map((item, i) => {
              ...
              return (
                <article key={`${item.texte}-${i}`} className="snap-start shrink-0 w-[250px] sm:w-[270px] rounded-2xl overflow-hidden flex flex-col" style={{ background: T.blanc, border: `1px solid ${T.bleuClairBord}`, boxShadow: "0 2px 8px rgba(15,23,42,.06)" }}>
                  ...
                  <div className="p-4 flex flex-col flex-1">
                    ...
                    <a href={whatsappHref(business.whatsapp, message)} target="_blank" rel="noopener noreferrer" className="mt-4 w-full inline-flex items-center justify-center gap-2 rounded-full py-2.5 text-xs font-bold text-white" style={{ background: "#25D366" }}>
      ...
      {/* 4. FOOTER */}
      <footer id="contact" className="scroll-mt-20" style={{ background: footerFonce }}>
        <div className="max-w-6xl mx-auto px-5 md:px-10 py-12">
          <div className="grid md:grid-cols-[1.5fr_1fr_1.2fr] gap-10">
            <div>
              ...
              {reseauxActifs.length > 0 && <div className="flex gap-2 mt-5">{reseauxActifs.map((r) => { const RIcon = r.icon; return <a key={r.id} href={business.reseaux[r.id]} target="_blank" rel="noopener noreferrer" aria-label={r.label || r.id} className="w-9 h-9 rounded-full flex items-center justify-center bg-white/10"><RIcon size={15} color="#fff" /></a>; })}</div>}
            </div>
            <div>
              <p className="text-xs font-bold uppercase tracking-wider text-white/45 mb-4">Navigation</p>
              <div className="flex flex-col gap-2.5"><a href="#accueil" className="text-sm text-white/75">Accueil</a><a href="#produits" className="text-sm text-white/75">{catalogueLabel}</a><a href="#contact" className="text-sm text-white/75">Contact</a></div>
            </div>
            <div>
              <p className="text-xs font-bold uppercase tracking-wider text-white/45 mb-4">Contact</p>
              <div className="space-y-3">
                {business.whatsapp && <a href={waGeneral} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm text-white/80"><MessageCircle size={15} /> {business.whatsapp}</a>}
                {business.email && <a href={`mailto:${business.email}`} className="flex items-center gap-2 text-sm text-white/80"><Mail size={15} /> {business.email}</a>}
          ...
          <div className="mt-10 pt-5 border-t border-white/10 flex flex-wrap items-center justify-between gap-3">
            ...
            {!paye && <a href="https://samasite.com" target="_blank" rel="noopener noreferrer" className="text-xs text-white/45">Créé avec <strong className="text-white/70">Sama Site</strong></a>}
          </div>
        </div>
      </footer>
    </div>
