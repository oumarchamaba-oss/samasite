        ...
        <div className="h-full overflow-y-auto" style={{ scrollbarWidth: "none" }}>
          ...
          <section id="produits" className="entree-douce px-4 py-7" style={{ animationDelay: "80ms" }}>
            ...
            <div className="flex gap-3 overflow-x-auto mt-4 pb-2" style={{ scrollbarWidth: "none" }}>
              {itemsAffiches.map((item, i) => <article key={item.id || `${item.texte}-${i}`} className="entree-douce carte-hover shrink-0 w-[155px] rounded-xl overflow-hidden" style={{ border: `1px solid ${T.bleuClairBord}`, background: "#fff", animationDelay: `${140 + Math.min(i, 6) * 60}ms` }}>
