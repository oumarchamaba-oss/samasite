import { SECTEURS, SECTEUR_COULEURS, RESEAUX_SOCIAUX, MODES_LIVRAISON, genererSchema, trouverMetier } from "./data";
...
export function genererFichierSite(site) {
  ...
  const items = normaliserItems(site.produits?.length ? site.produits : demoActif.produits);
  ...
  const couleursDispo = SECTEUR_COULEURS[secteur.id] || SECTEUR_COULEURS.service;
  ...
  const imageHero = site.banniere_url || site.banniere;
  ...
  const cardsHTML = items.map((item, i) => {
    ...
    return `<article class="card" data-category="${attr(item.categorie || "Tous")}">${image}<div class="card-body"><h3>${esc(item.texte)}</h3>${item.description ? `<p class="desc">${esc(item.description)}</p>` : ""}${item.prix ? `<p class="price">${esc(item.prix)}</p>` : ""}<a class="order" data-base="${attr(normaliserNumero(whatsapp))}" data-message="${attr(messageBase)}" href="${attr(wa(whatsapp, messageBase + "."))}" target="_blank" rel="noopener noreferrer">${esc(actionLabel)} <span>↗</span></a></div></article>`;
  }).join("");
...
<main>
<section id="accueil" class="hero"><div class="hero-copy"><div class="eyebrow">Bienvenue chez ${esc(nom)}</div><h1>${heroTitle}</h1><p>${site.metier ? `${esc(site.metier)} — ` : ""}${estService ? "Des services pensés pour répondre simplement à vos besoins." : "Des produits sélectionnés avec soin pour vous."}</p><a class="hero-cta" href="${attr(generalWa)}" target="_blank" rel="noopener noreferrer">${esc(actionLabel)} sur WhatsApp ↗</a></div><div class="hero-media">${imageHero ? `<img src="${attr(imageHero)}" alt="${attr(nom)}">` : `<div class="placeholder">${esc(nom.slice(0,1).toUpperCase())}</div>`}${!site.paiement_confirme ? `<div class="credit">Créé avec Sama Site</div>` : ""}</div></section>
