...
// Construit le champ "icons" des métadonnées Next.js à partir du logo du
// commerce, pour que l'onglet du navigateur (et l'icône si le client
// "ajoute à l'écran d'accueil" sur son téléphone) affiche le logo du
// commerce plutôt que l'icône générique de Sama Site (app/icon.svg), sur
// les deux pages qui rendent un site (voir app/s/[slug]/page.js et
// app/site/[token]/layout.js). Le logo est stocké en base64 (data URL,
// voir lireImage() dans EditerSite.js/CreerSite.js) — un favicon en data
// URL est standard, supporté par tous les navigateurs.
export function iconesDepuisLogo(logoUrl) {
  if (!logoUrl) return undefined;
  const type = logoUrl.match(/^data:([^;,]+)[;,]/)?.[1];
  return { icon: type ? { url: logoUrl, type } : logoUrl };
}
